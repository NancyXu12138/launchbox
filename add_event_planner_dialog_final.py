...
# 在技能选择弹窗之后添加Event Planner Dialog
if "Event Planner 表单弹窗" not in content:
    # 找到技能选择弹窗的结束位置
    skill_dialog_end = content.find("</Dialog>\n\n          {/* 图片放大弹窗 */}")
    if skill_dialog_end != -1:
        # 在技能选择弹窗后添加Event Planner Dialog
        insertion_point = skill_dialog_end + len("</Dialog>")
        content = content[:insertion_point] + dialog_content + content[insertion_point:]
        print("Added Event Planner Dialog!")
    else:
        print("Could not find insertion point for Event Planner Dialog")
else:
    print("Event Planner Dialog already exists")

# 写入文件
with open('src/modules/chat/ChatPage.tsx', 'w', encoding='utf-8') as f:
    f.write(content)

print("Event Planner Dialog processing complete!")
