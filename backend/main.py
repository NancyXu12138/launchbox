...
class EventPlanningRequest(BaseModel):
    ...
    overview: str
    businessGoal: str
    businessGoalCustom: str = ""
    targetPlayer: str
    targetPlayerCustom: str = ""
    targetRegion: str

@app.post("/api/generate-event-plan")
async def generate_event_plan(request: EventPlanningRequest):
    """生成活动策划案"""
    try:
        from services.event_planning_service import event_planning_service
        
        form_data = {
            'theme': request.theme,
            'overview': request.overview,
            'businessGoal': request.businessGoal,
            'businessGoalCustom': request.businessGoalCustom,
            'targetPlayer': request.targetPlayer,
            'targetPlayerCustom': request.targetPlayerCustom,
            'targetRegion': request.targetRegion
        }
        
        result = await event_planning_service.generate_event_plan(form_data)
        return result
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"生成活动策划案失败: {str(e)}")
